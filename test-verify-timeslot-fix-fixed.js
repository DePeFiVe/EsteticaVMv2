...
async function testTimeSlotFix() {
  console.log('=== INICIANDO TEST DE VERIFICACIÓN DE CORRECCIÓN DE FECHAS ===');
  ...
  try {
    ...
    const startTimeISO = `${tomorrowStr}T${slot.startTime}:00.000Z`;
    const endTimeISO = `${tomorrowStr}T${slot.endTime}:00.000Z`;
    ...
    if (tomorrowCheckError) {
      ...
      if (todayResults && todayResults.length > 0) {
        ...
        todayResults.forEach(slot => {
          // Extraer la hora directamente del string ISO, ignorando la zona horaria
          const startParts = slot.start_time.split('T')[1].split(':');
          const endParts = slot.end_time.split('T')[1].split(':');
          ...
          console.log(`     Fin (ISO): ${slot.end_time}`);
          console.log(`     Fin (HH:MM): ${endTime}`);
          console.log(`     Razón: ${slot.reason}`);
          console.log(`     ¿Coinciden los horarios?: ${slot.reason.includes(`${startTime} - ${endTime}`) ? '✅ SÍ' : '❌ NO'}`);
        });
      } else {
