...
// Nota: Este script debe ejecutarse con Node.js y requiere que las variables de entorno estén configuradas
...
async function verificarFuncionesSql() {
  try {
    // Importar dinámicamente para evitar problemas con ESM/CommonJS
    const { supabaseAdmin } = await import('./src/lib/supabase.ts');
    
    if (!supabaseAdmin) {
      console.error('❌ Error: Cliente administrativo no disponible');
      console.log('Asegúrate de que la variable de entorno SUPABASE_SERVICE_ROLE_KEY esté configurada.');
      return;
    }
